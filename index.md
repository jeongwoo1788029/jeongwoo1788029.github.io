...
- 11 마우스 스크롤로 아이템을 선택할 수 있다.<br>
    -> 번호 입력으로 아이템 선택 변경 고려.<br>
    
- 19 체력이 0이 될 시 방 시작점에서 리스폰<br>
    -> 방마다 스폰 포인트 지정.<br>
    -> 방 입장 시(문 통과 ) 스폰 포인트 변경.<br>
     
     
     
# [4주차 작업 결과] <a name='14'></a>
작업명 : 문과 상호작용
...
- 18번 요구사항 레이져 충돌 시 체력감소 - [100%] 추가    
- 번호 입력 4자리로 제한(수정 가능)
- 번호키 나가기버튼은 쪽지 나가기버튼에서 "문과 상호작용 (불)가능" 메시지가 없어지는 기능을 추가했습니다.     
- RaycastHit 관련 NullReferenceException 오류 해결
    + Ray(시선)가 특정 오브젝트에 히트가 되어 상호작용하면 오류가 뜨지않지만, 특정 오브젝트가 아닌 오브젝트에 맞고 상호작용(E키)을 하면 NullReferenceException 오류가 발생했다.
    + "특정 오브젝트와 Ray와 히트가 된다면"이라는 조건문을 넣어 해결
...
- 플레이어 회전 문제 해결
- 방마다 오브젝트 배치 및 추가(방마다 다르게 구성)
- 스토리 연결에 대해 복선이나 암시, 엔딩 아이디어
- 힌트 추가 및 배치
- 3D TEXT 가 오브젝트를 통과하는 문제 해결

# [1차 최종 작업 결과] <a name='18'></a>
