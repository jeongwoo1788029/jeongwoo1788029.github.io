...
### 10. [용어 정리](#10)
### 11. [개발 작업](#11)
### 12. [1주차 작업 결과](files/w01/index.md)
### 13. [2주차 작업 결과](files/w01/w02.md)
### 14. [3주차 작업 결과](files/w01/w03.md)
### 15. [4주차 작업 결과](files/w01/w04.md)
### 16. [5주차 작업 결과](files/w01/w05.md)
### 17. [6주차 작업 결과](files/w01/w06.md)
...
# [요구사항 1년차] <a name='7'></a>
 ...
 - w,a,s,d를 누를 시 앞쪽,왼쪽,뒤쪽,앞쪽으로 일정 속도로 이동한다.
   + 걸을 때 걷는음악이 재생된다.
 - spacebar를 누를 시 일정 높이를 점프한다.(2단점프)
   + 점프할 때 점프음악이 재생된다.
 - 마우스로 보는 방향을 전환한다.
 - 마우스 스크롤로 아이템을 선택할 수 있다.  
 
 
 - 특정 오브젝트(쪽지, 문, 번호키)와 'e'키로 상호작용할 수 있다.
   + 특정 오브젝트에 다가갈 시 화면 중앙 하단에 'e' 메시지가 뜬다.  
 
 
 - 쪽지와 상호작용하면 글자가 써져있는 창이 뜬다.
   + 'e'키를 누를 시 다시 창이 사라진다.
 - 열쇠를 들고 있으면 문과 상호작용할 수 있다.
 - 번호키와 상호작용하면 마우스로 누를 수있는 '1~9의 숫자'와 '나가기' 버튼이 있다.
   + 정답 번호를 누를 시 '문을 열수 있다.'라는 메시지가 뜬다. -> 문과 상호작용할 수 있다.
   + '나가기' 버튼을 왼쪽 클릭 시 창이 닫힌다.  
 
 
 - 원거리 몬스터의 공격을 받을 시 현재체력이 최대체력의 1/10이 줄어든다.
 - 레이져와 접촉 할 시 현재체력이 최대체력의 1/3이 줄어든다.
 - 낭떠러지로 떨어지면 체력이 0이 된다.
 - 체력이 0이 되면 죽었던 방 안에서 다시 스폰된다.(스폰포인트)
 - 포션과 접촉 시 현재체력이 최대체력의 1/3만큼 회복된다.  
 
 
 - 발판 및 레이져 생성
 - 생성과 제거가 일정시간을 간격으로 반복되는 발판 및 레이져가 있다.
 - 눈에 보이지만 밟을 수 없는 발판 및 레이져가 존재한다.
 - 움직이는 발판 및 레이져가 있다.  
